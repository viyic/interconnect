...
function create_entity(x, y, type, entity)
  ...
  return table.insert(entities,
    { id = id, x = x, y = y, w = w, h = h, type = type, ["drill-type"] = drill_type, durability = durability })
end
...
function get_entity_id_from_position(x, y, ix)
  ...
  if entity == nil then
    return 0
  elseif point_in_rectangle(x, y, entity.x, entity.y, (entity.x + (entity.w - 1)), (entity.y + (entity.h - 1))) then
    return entity.id
  else
...
function update(dt, set_mode)
  ...
  if building ~= 0 then
    ...
    if build == "connector" then
      if entity ~= nil and entity.type ~= "gold" and entity.type ~= "fuel" then
        building_obstructed = false
      end

      local from = entities[get_entity_index_from_id(building_from)]
      local to = entity or { x = building_x, y = building_y, w = 1, h = 1 }
      if from ~= nil then
        building_price = lume.round((lume.distance((from.x + (from.w / 2)), (from.y + (from.h / 2)), (to.x + (to.w / 2)), (to.y + (to.h / 2))) * connector_price))
      end
    end

    if build == "processor" then
      if entity == nil and gold >= processor_price then
        building_obstructed = false
      end
    end
...
function draw(set_mode)
  ...
  for index = 1, 10 do
    ...
    for inner_index = 0, 35 do
      draw_line(x + (index * grid_size), y + (inner_index * 25), x + (index * grid_size), y + (inner_index * 25) + 12.5,
        3)
    end
    for inner_index = 0, 45 do
      draw_line(x + (inner_index * 25), y + (index * grid_size), x + (inner_index * 25) + 12.5, y + (index * grid_size),
        3)
    end
  end
  ...
  if building ~= 0 then
    ...
    if build == "connector" then
      ...
      if building_from ~= 0 then
        ...
        local from_x = (from.x + (from.w / 2)) * grid_size
        local from_y = (from.y + (from.h / 2)) * grid_size
        local to_x = (building_x + 0.5) * grid_size
        local to_y = (building_y + 0.5) * grid_size
        love.graphics.setColor(1, 0, 0.2)
        love.graphics.circle("fill", (x1 + (grid_size / 2)), (y1 + (grid_size / 2)), 10)
        love.graphics.circle("fill", from_x, from_y, 10)
        draw_line(from_x, from_y, to_x, to_y, 5)

        if building_price ~= 0 then
          local text_x = from_x + (to_x - from_x) / 2
          local text_y = from_y + (to_y - from_y) / 2
          local text = "gold " .. building_price
          love.graphics.setColor(rgb(228, 196, 21))
          draw_rectangle_centered("fill", text_x, text_y, font:getWidth(text) + 4, font:getHeight() + 4, 5)
          love.graphics.setColor(1, 1, 1)
  ...
  for index, build in ipairs(builds) do
    ...
    draw_text((build_x + (build_w / 2)), ((build_y + (build_h / 2)) + small_font:getHeight()),
      ("gold " .. build_prices[index]))
